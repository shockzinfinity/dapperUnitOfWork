...
public class Retry
{
  public static T Do<T>(Func<T> func, RetryOptions retryOptions)
  {
    ...
    if (retryOptions?.ExceptionDetector == null) return func();
    ...
    while (retryCount <= retryOptions.MaxRetries) {
      try {
        ...
        Task.Run(() => HandleExceptionAsync(retryOptions, ex, retryCount)).Wait();
      }
  ...
  public static void Do(Action action, RetryOptions retryOptions) => Do(() =>
  {
    action();
    return true;
  }, retryOptions);

  public static async Task DoAsync(Func<Task> action, RetryOptions retryOptions) => await DoAsync(async () =>
  {
    await action();
    return true;
  }, retryOptions);
